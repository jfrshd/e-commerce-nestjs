...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    ...
    // Check if there are multiple users with the same email (shouldn't happen due to unique constraint)
  ...
  async updatePreferences(userId: string, updatePreferencesDto: UpdatePreferencesDto) {
    ...
    if (updatePreferencesDto.language !== undefined) user.language = updatePreferencesDto.language;
    if (updatePreferencesDto.notificationsEnabled !== undefined) user.notificationsEnabled = updatePreferencesDto.notificationsEnabled;
    if (updatePreferencesDto.location !== undefined) user.location = updatePreferencesDto.location;
    if (updatePreferencesDto.theme !== undefined) user.theme = updatePreferencesDto.theme;
    await this.userRepository.save(user);
